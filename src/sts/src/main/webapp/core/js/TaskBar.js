...
Ext.extend(JW.TaskBar, Ext.util.Observable, {
    init : function(){
        ...
        this.trayPanel = new JW.TaskButtonsPanel({
            el: 'ux-traybuttons-panel',
            id: 'TaskBarTrayButtons',
            region:'east',
            width: 64
        });
    ...
    addTrayButton : function(win){
        return this.trayPanel.addTrayButton(win);
    },
...
JW.TaskButtonsPanel = Ext.extend(Ext.BoxComponent, {
    ...
    /**
     * Add tray lunch button
     */
    addTrayButton : function(win) {
        var qLi = this.strip.createChild({tag:'li', cls: 'jw-traybutton'}, this.edge); // insert before the edge
        var qBtn = new JW.TaskBar.TrayButton(win, qLi);
        //this.items.push(qBtn); 
    },
...
/**
 * @class JW.TaskBar.TrayButton
 * @extends Ext.Button
 */
JW.TaskBar.TrayButton = function(win, el){
	this.win = win;
	JW.TaskBar.QuickButton.superclass.constructor.call(this, {
        iconCls: win.initialConfig.iconCls,
        //text: Ext.util.Format.ellipsis(win.title, 12),
		tooltip: win.title,
        renderTo: el,
        handler : function(){
...
Ext.extend(JW.TaskBar.TrayButton, Ext.Button, {});
/**
